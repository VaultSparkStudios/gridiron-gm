...
setScPts(pr=>pr+2+repB);setTeams(nt);setSched(ns);setWk(nw);if(moraleEvLogs.length)setLog(l=>[...moraleEvLogs,...l.slice(0,150-moraleEvLogs.length)]);if(ur){setLog(p=>[...p,ur,...(lkrEv?[`🎭 ${lkrEv.txt}`]:[])]);sm(ur);}else if(lkrEv)setLog(p=>[...p,`🎭 ${lkrEv.txt}`]);if(!trProp&&nw<=10&&Math.random()<(nw>=8?0.44:0.22)){const off=genTradeOffer(nt,ui,draftPicks);if(off)setTrProp(off);}
...
if(nw===18&&ownerGoals&&!ownerGoals.evaluated){const uW=nt[ui].w;const met=uW>=(ownerGoals.wins||8);if(met){nt[ui].gmRep=cl((nt[ui].gmRep||50)+3,0,100);setLog(l=>[`🏆 OWNER: Met ${ownerGoals.wins}-win target (${uW}W) — gmRep +3`,...l.slice(0,149)]);}else{nt[ui].gmRep=cl((nt[ui].gmRep||50)-5,0,100);setLog(l=>[`😤 OWNER PRESSURE: ${uW}W — missed ${ownerGoals.wins}-win target — gmRep -5`,...l.slice(0,149)]);}setOwnerGoals(g=>g?{...g,evaluated:true}:g);}
