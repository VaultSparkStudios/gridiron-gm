...
// Cap violation check
nt.forEach((t,i)=>{if(capHit(t)>CAP_CEILING){t.deadCap=(t.deadCap||0)+5;if(i===ui){const pk3=draftPicks.find(pk=>pk.owner===ui&&pk.rd===3);if(pk3)setDraftPicks(pr=>pr.filter(p=>p.id!==pk3.id));setLog(l=>[`⚠️ CAP VIOLATION: ${t.city} over $${CAP_CEILING}M — $5M fine + ${pk3?'3rd-round pick forfeited':'no 3rd pick to forfeit'}`,...l.slice(0,149)]);}}});const ns=[...sched];let ur="";ns.filter(g=>g.wk===nw&&!g.played).forEach(g=>{const gi=ns.indexOf(g);const _fqbH=g.h===ui&&franchiseQB?{...nt[g.h],_fqbBonus:5}:nt[g.h];const _fqbA=g.a===ui&&franchiseQB?{...nt[g.a],_fqbBonus:5}:nt[g.a];let r=simGame(_fqbH,_fqbA,g.h===ui?gamePlan:{off:'balanced',def:'balanced'},g.a===ui?gamePlan:{off:'balanced',def:'balanced'});// FEATURE 2: Stadium effects
...
const repB=(nt[ui].gmRep||50)>=90?2:(nt[ui].gmRep||50)>=60?1:0;const lkrEv=Math.random()<0.15?LKR_EVENTS[R(0,LKR_EVENTS.length-1)]:null;if(lkrEv){nt[ui].morale=cl((nt[ui].morale||50)+lkrEv.mrl,0,100);nt[ui].roster.forEach(p=>{if(lkrEv.confP>0&&Math.random()<0.4)p.conf=cl((p.conf||60)+lkrEv.confP,0,100);if(lkrEv.confN<0&&Math.random()<0.35)p.conf=cl((p.conf||60)+lkrEv.confN,0,100);});}// MOVE A: Player Morale Events
