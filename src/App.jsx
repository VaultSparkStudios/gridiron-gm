...
function initPicks(teams,yr){
  ...
  const picks=[];
  const COND_POOL=['if team wins 8+ games','if team makes playoffs','if QB starts 14+ games'];
  for(let rd=1;rd<=7;rd++)order.forEach((tid,i)=>{const condition=rd>=4&&Math.random()<0.12?COND_POOL[Math.floor(Math.random()*COND_POOL.length)]:null;picks.push({id:uid(),rd,num:i+1,overall:(rd-1)*32+i+1,owner:tid,orig:tid,yr,condition});});
  return picks;
}
...
const[fourthChoice,setFourthChoice]=useState(null);const[showDepth,setShowDepth]=useState(false);const[showPS,setShowPS]=useState(false);const[showIR,setShowIR]=useState(false);const[preseasonGames,setPreseasonGames]=useState([]);
  ...
  const[faLostCount,setFaLostCount]=useState(0);
...
const _rivH=g.h===ui&&rivalry&&g.a===rivalry?.teamId;const _rivA=g.a===ui&&rivalry&&g.h===rivalry?.teamId;const _rivBoostH=_rivH?{..._fqbH,_rivBonus:4}:_fqbH;const _rivBoostA=_rivA?{..._fqbA,_rivBonus:4}:_fqbA;let r=simGame(_rivBoostH,_rivBoostA,g.h===ui?gamePlan:{off:'balanced',def:'balanced'},g.a===ui?gamePlan:{off:'balanced',def:'balanced'});// FEATURE 2: Stadium effects
...
const moraleEvLogs=[];nt.forEach(t=>t.roster.forEach(p=>{if(p.injured&&p.injSev==="major"&&!(t.ir||[]).find(r=>r.id===p.id)){if((t.ir||[]).length<8){const irWk=nw;t.ir=[...(t.ir||[]),{...p,irWk,irMin:6}];t.roster=t.roster.filter(r=>r.id!==p.id);moraleEvLogs.push(`🚑 MAJOR INJURY: ${p.name} placed on IR (${p.injType})`);}}}));nt.forEach(t=>t.roster.forEach(p=>{p.av=calcAV(p);}));// PRACTICE SQUAD AUTO-RETURN
...
const uStr=nt[ui].streak||0;if(uStr<=-3){const hsRoles=['oc','dc','st'].filter(r=>nt[ui].coach?.[r]);setHotSeat(hsRoles);if(uStr<=-4){const worst=['oc','dc','st'].filter(r=>nt[ui].coach?.[r]).sort((a,b)=>(nt[ui].coach[a]?.rating||0)-(nt[ui].coach[b]?.rating||0))[0];if(worst){const fired=nt[ui].coach[worst];setFaCoaches(pr=>[...pr,fired]);nt[ui].coach={...nt[ui].coach,[worst]:null};setHotSeat([]);setLog(l=>[`🔥 FIRED: ${worst.toUpperCase()} ${fired.name} — losing streak`,...l.slice(0,149)]);}}else{moraleEvLogs.push(`🔥 HOT SEAT WARNING — staff under pressure`);}}else if(uStr>0)setHotSeat([]);
...
const aiSigned=new Set();let _faLost=0;nt.forEach((t,idx)=>{if(idx===ui)return;POS.forEach(pos=>{const have=t.roster.filter(p=>p.pos===pos).length;const need=(rm[pos]||2)-have;for(let s=0;s<Math.max(0,need);s++){const top=expFAs.find(f=>f.pos===pos&&!aiSigned.has(f.id));if(!top)break;if(teams[ui].roster.find(r=>r.id===top.id))_faLost++;aiSigned.add(top.id);t.roster.push({...top,salary:+(top.ovr/99*Rf(2,8)).toFixed(1),contract:R(1,3),ss:emptySS(top.pos),gl:[],av:0});}});});if(_faLost>0)setFaLostCount(f=>f+_faLost);
...
// v12: comp picks for FA losses
if((nt[ui].gmRep||50)>=60&&faLostCount>0){const cpRd='R'+R(3,5);setCompPickQueue(q=>[...q,cpRd]);setLog(l=>[`📋 Comp Pick Awarded: ${cpRd} (lost ${faLostCount} FA${faLostCount>1?'s':''})`,...l.slice(0,149)]);}setFaLostCount(0);
