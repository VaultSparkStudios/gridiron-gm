...
function teamStr(t){const r=t.roster.filter(p=>!p.injured&&!p.holdout&&!p.suspended);if(!r.length)return 50;const st={QB:1,RB:1,WR:3,TE:1,LT:1,LG:1,C:1,RG:1,RT:1,DL:4,LB:3,CB:2,S:2,K:1};const a=[];for(const[pos,n]of Object.entries(st)){const ap=r.filter(p=>p.pos===pos).sort((x,y)=>y.ovr-x.ovr);a.push(...ap.slice(0,n));}const base=a.length?a.reduce((s,p)=>s+p.ovr,0)/a.length:50;const chemBonus=((t.chemistry||75)-75)*0.2;let depthPenalty=0;for(const[pos,n]of Object.entries(st)){const available=r.filter(p=>p.pos===pos);if(available.length===0)depthPenalty+=1.5;else if(available.length<=n)depthPenalty+=0.5;}const cyBoost=a.reduce((s,p)=>s+(p.contract===1?2:0),0)/Math.max(a.length,1);const avgFit=a.length?a.reduce((s,p)=>s+(p.fit||70),0)/a.length:70;const coreBonus=a.filter(p=>p.role==='core').length*0.5;
  return base+chemBonus-depthPenalty+cyBoost+(avgFit-70)*0.05+(t._fqbBonus||0)+(t._rivBonus||0)+coreBonus;}
...
if(injInsurance.includes(p.id)){t.deadCap=Math.max(0,(t.deadCap||0)-2);setInjInsurance(ins=>ins.filter(x=>x!==p.id));moraleEvLogs.push(`🛡️ INSURANCE: ${p.name} IR — $2M cap relief`);}else moraleEvLogs.push(`🚑 MAJOR INJURY: ${p.name} placed on IR (${p.injType})`);}}}));nt.forEach(t=>t.roster.forEach(p=>{p.av=calcAV(p);}));// Injury drama arc — weekly recovery story events
...
const fanSatSpMod=fanSat<30?-1:fanSat>70?1:0;setFanSat(fs=>cl(fs+fsD,0,100));setScPts(pr=>pr+2+repB+fanSatSpMod);setTeams(nt);setSched(ns);setWk(nw);
...
nt[ui].chemistry=cl((nt[ui].chemistry||75)-trOff.g.length*2+trOff.r.length,0,100);setTeams(nt);setDraftPicks(np);const _tDesc=`Sent ${trOff.g.map(p=>p.name).join('+')||'pick'} to ${TEAMS[trTm].ab}, got ${trOff.r.map(p=>p.name).join('+')||'pick'}`;setTradeHistory(h=>[{wk,giveNames:trOff.g.map(p=>p.name),getNames:trOff.r.map(p=>p.name),desc:_tDesc},...h].slice(0,20));setLog(p=>[...p,`Trade with ${TEAMS[trTm].ab}`]);sm("Trade completed!");setTrOff({g:[],r:[],gPk:[],rPk:[]});};
  ...
  // ═══ P5: GM ↔ PLAY BRIDGE ═══
  const exportGameToPlay=(game)=>{const oppId=game.h===ui?game.a:game.h;const opp=teams[oppId];const ut=teams[ui];const fmt=t=>({name:`${t.city} ${t.name}`,ab:t.ab,clr:t.clr,ac:t.ac,ocScheme:t.coach?.oc?.scheme||null,dcScheme:t.coach?.dc?.scheme||null,record:`${t.w}-${t.l}`,players:t.roster.filter(p=>!p.injured).map(p=>({id:p.id,name:p.name,pos:p.pos,ovr:p.ovr,spd:p.spd||70,str:p.str||70,jmp:p.jmp||70,personality:p.personality||null,salary:p.salary||0}))});const data={team:fmt(ut),opponent:fmt(opp),week:game.wk,season:yr,gameId:`${game.wk}-${game.h}-${game.a}`,stadiumUpgrades:stadium?.upgrades||[]};try{localStorage.setItem('gm_roster_export',JSON.stringify(data));window.open('https://vaultsparkstudios.com/gridiron-gm-play/','_blank');sm(`Week ${game.wk} vs ${opp.ab} exported → Play`);track('play_exported');}catch{sm("Export failed");}};
  const exportToPlay=()=>{const ut=teams[ui];const opp=teams[(ui+1)%teams.length];const fmt=t=>({name:`${t.city} ${t.name}`,ab:t.ab,clr:t.clr,ac:t.ac,ocScheme:t.coach?.oc?.scheme||null,dcScheme:t.coach?.dc?.scheme||null,record:`${t.w}-${t.l}`,players:t.roster.filter(p=>!p.injured).map(p=>({id:p.id,name:p.name,pos:p.pos,ovr:p.ovr,spd:p.spd||70,str:p.str||70,jmp:p.jmp||70,personality:p.personality||null}))});try{localStorage.setItem('gm_roster_export',JSON.stringify({team:fmt(ut),opponent:fmt(opp),stadiumUpgrades:stadium?.upgrades||[]}));sm("Exported to Play ✓");}catch{sm("Export failed");}};
...
if(d.passYds||d.pyds){p.ss.passYds=(p.ss.passYds||0)+(d.passYds||d.pyds||0);p.ss.passTD=(p.ss.passTD||0)+(d.passTD||d.ptds||0);p.ss.passInt=(p.ss.passInt||0)+(d.passInt||d.ints||0);p.ss.att=(p.ss.att||0)+(d.att||Math.ceil((d.passYds||d.pyds||0)/8));p.ss.comp=(p.ss.comp||0)+(d.comp||Math.ceil((d.passYds||d.pyds||0)/12));}if(d.rushYds||d.ryds){p.ss.rushYds=(p.ss.rushYds||0)+(d.rushYds||d.ryds||0);p.ss.rushAtt=(p.ss.rushAtt||0)+(d.rushAtt||Math.ceil((d.rushYds||d.ryds||0)/4));p.ss.rushTD=(p.ss.rushTD||0)+(d.rushTD||d.rtds||0);}if(d.recYds||d.reyds){p.ss.recYds=(p.ss.recYds||0)+(d.recYds||d.reyds||0);p.ss.rec=(p.ss.rec||0)+(d.rec||Math.ceil((d.recYds||d.reyds||0)/9));p.ss.recTD=(p.ss.recTD||0)+(d.recTD||d.retds||0);}if(d.tack){p.ss.tkl=(p.ss.tkl||0)+d.tack;}if(d.sacks){p.ss.sacks=(p.ss.sacks||0)+d.sacks;}p.ss.gp=(p.ss.gp||0)+1;// B1: store per-game grade in player game log
if(d.grade){if(!p.gl)p.gl=[];p.gl.unshift({wk:res.week||0,g:d.grade});if(p.gl.length>20)p.gl=p.gl.slice(0,20);}liveStatsSynced=true;}});(res.injuries||[]).forEach(inj=>{const p=nt[ui].roster.find(r=>r.id===inj.id);if(p&&!p.injured){p.injured=true;p.injWk=inj.weeks;p.injType="Game injury";}});const ts=res.score?.team||0,os=res.score?.opp||0;if(ts>os)nt[ui].w++;else if(os>ts)nt[ui].l++;else nt[ui].t++;nt[ui].pf+=ts;nt[ui].pa+=os;setTeams(nt);// Mark the matching scheduled game as played
...
<label style={{cursor:"pointer",display:"block",marginTop:8}}><span style={{display:"inline-block",background:"#1e293b",color:C.mt,padding:"8px 24px",borderRadius:8,fontWeight:700,fontSize:12,cursor:"pointer"}}>📂 LOAD GAME</span><input type="file" accept=".json" style={{display:"none"}} onChange={loadGame}/></label></div></div>);
  ...
  return(<div style={{minHeight:"100vh",background:C.bg,fontFamily:C.f,color:C.tx,display:"flex",flexDirection:"column"}}>
    ...
    {tab==="roster"&&ut&&<div>
      ...
      {sp==="preseason"&&preseasonGames.length===0&&<div style={{background:C.cd,border:`1px solid ${C.bd}`,borderRadius:5,padding:"6px 8px",marginBottom:6}}><div style={{fontSize:9,fontWeight:700,color:C.gd,marginBottom:4}}>🏈 PRESEASON — 2 Exhibition Games</div><div style={{fontSize:8,color:C.mt,marginBottom:4}}>Sim preseason games to evaluate depth players. No W/L impact.</div><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><button onClick={()=>setPreseasonRisk(r=>!r)} style={{background:preseasonRisk?"#7f1d1d22":"#14532d22",color:preseasonRisk?"#fca5a5":"#86efac",border:`1px solid ${preseasonRisk?"#ef4444":"#22c55e"}44`,borderRadius:3,padding:"2px 7px",fontSize:8,fontWeight:700,cursor:"pointer"}}>⚠️ Injury Risk: {preseasonRisk?"ON":"OFF"}</button><span style={{fontSize:7,color:C.mt}}>{preseasonRisk?"8% starter injury chance":"No injury risk"}</span></div><button onClick={()=>{const opp1=teams[(ui+1)%teams.length],opp2=teams[(ui+2)%teams.length];const g1=simGame(teams[ui],opp1),g2=simGame(teams[ui],opp2);const results=[{opp:opp1.ab,us:g1.hsc,them:g1.asc},{opp:opp2.ab,us:g2.hsc,them:g2.asc}];if(preseasonRisk){const nt=[...teams];const starters=nt[ui].roster.filter(p=>!p.injured).sort((a,b)=>b.ovr-a.ovr).slice(0,22);starters.forEach(p=>{if(Math.random()<0.08){p.injured=true;p.injWk=wk;p.injType="Minor";p.injSev="minor";p.injRecWks=1;p.injDue=wk+1;}});setTeams(nt);}setPreseasonGames(results);const depthStars=teams[ui].roster.filter(p=>p.ovr<75).sort(()=>Math.random()-.5).slice(0,3).map(p=>`${p.name} (${p.pos})`);setLog(pr=>[...pr,`📋 Preseason: ${results[0].us}-${results[0].them} vs ${results[0].opp}, ${results[1].us}-${results[1].them} vs ${results[1].opp}`,`👀 Standout depth: ${depthStars.join(', ')}`]);sm("Preseason complete!");}} style={{background:`${C.gn}22`,color:C.gn,border:`1px solid ${C.gn}44`,borderRadius:4,padding:"4px 12px",fontSize:9,fontWeight:700,cursor:"pointer"}}>▶ Sim Preseason</button></div>}
    ...
    {tab==="trade"&&<div>
      ...
      {tradeHistory.length>0&&<div style={{marginTop:10,background:C.cd,borderRadius:5,border:`1px solid ${C.bd}`,padding:"6px 10px"}}><div style={{fontSize:9,fontWeight:700,color:C.gd,marginBottom:4}}>📋 TRADE HISTORY (last {tradeHistory.length})</div>{tradeHistory.map((h,i)=><div key={i} style={{fontSize:8,color:C.mt,padding:"2px 0",borderBottom:`1px solid ${C.bd}22`}}><span style={{color:"#94a3b8",marginRight:4}}>Wk{h.wk}</span>{h.desc}</div>)}</div>}
    </div>}
    ...
    {tab==="draft"&&<div>
      ...
      {sp==="draft"&&draftActive&&curPick?<div>
        ...
        {curPick.owner===ui&&<div style={{display:'flex',alignItems:'center',gap:6,marginBottom:5}}><span style={{fontSize:9,color:C.gn,fontWeight:700}}>🎯 Your pick!</span><Btn onClick={genDraftPickTrade} bg="#7c3aed" c="#e9d5ff" style={{fontSize:8,padding:'2px 8px'}}>📤 Trade Pick</Btn></div>}
