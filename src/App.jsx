...
const PlayerTable=({players,setSel,sortCol,sortDir,onSort,posFilter,setPosFilter,showSign,onSign})=>{
  ...
  return(<div>
    ...
    <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}><thead><tr>
      ...
      <td style={{padding:"2px"}}>{p.injured&&<span style={{color:p.injSev==='major'?C.rd:p.injSev==='moderate'?"#f97316":"#eab308",fontSize:7,fontWeight:800,marginRight:1}}>{p.injSev==='major'?"MAJ":p.injSev==='moderate'?"MOD":"MIN"} </span>}<PN p={p} setSel={setSel}/>{p.age<=24&&(p.pot||0)-p.ovr>=12&&<span style={{color:"#22d3ee",fontSize:7,fontWeight:800,marginLeft:2}}>⬆</span>}{p.traits?.[0]&&<span style={{fontSize:6,background:'#1e3a5f',color:'#93c5fd',borderRadius:2,padding:'1px 3px',marginLeft:2}}>{p.traits[0]}</span>}</td>
...
if(sp==='regular'&&!aiOffer){const aiIdxs=nt.map((_,i)=>i).filter(i=>i!==ui);for(const tid of aiIdxs){const isBuyer=nt[tid].w>=6&&nw>=10&&nw<=11;const ch=isBuyer?0.35:0.20;if(Math.random()<ch){const aiTeam=nt[tid];const posNeeds=POS.map(pos=>({pos,cnt:aiTeam.roster.filter(p=>p.pos===pos&&p.ovr>=65).length})).sort((a,b)=>a.cnt-b.cnt);const neededPos=posNeeds.slice(0,3).map(x=>x.pos);const targets=nt[ui].roster.filter(p=>neededPos.includes(p.pos)&&p.ovr>=72&&p.ovr<=88&&!p.ftag);if(!targets.length)continue;const wantedP=targets[R(0,targets.length-1)];const wv=wantedP.tradeVal;const isSeller=nt[tid].w<=4;const valMult=isSeller?1.15:0.95;const offered=aiTeam.roster.filter(p=>p.tradeVal>=wv*0.65*valMult&&p.tradeVal<=wv*1.25*valMult&&p.id!==wantedP.id).sort((a,b)=>Math.abs(a.tradeVal-wv)-Math.abs(b.tradeVal-wv)).slice(0,2);if(!offered.length)continue;setAiOffer({tid,offeredPids:offered.map(p=>p.id),wantedPid:wantedP.id,expires:nw+2});break;}}
}
  ...
  const PM=({p,onClose})=>{if(!p)return null;const ss=p.ss||{};const dO=p.scoutLvl>=2?p.trueOvr:p.scoutedOvr||p.ovr;const dP=p.scoutLvl>=2?p.truePot:p.scoutedPot||p.pot;
    return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:12}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.cd,borderRadius:14,padding:16,maxWidth:500,width:"100%",maxHeight:"85vh",overflowY:"auto",border:`1px solid ${C.bd}`}}>
        ...
        {p.traits&&p.traits.length>0&&<div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:6}}>{p.traits.map((tr,i)=><span key={i} style={{fontSize:8,background:'#1e3a5f',color:'#93c5fd',borderRadius:3,padding:'2px 6px',fontWeight:700}}>{tr}</span>)}</div>}
