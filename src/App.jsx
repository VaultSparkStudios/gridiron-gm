...
function aiBestPick(roster,available){
  ...
  // FEATURE 14: 70/30 need vs BPA weighting
  if(Math.random()<0.7){for(const pos of needs){const bp=available.find(p=>p.pos===pos);if(bp)return bp;}}
...
function initTeams(ui){return TEAMS.map((t,i)=>({...t,id:i,isUser:i===ui,roster:genRoster(),ps:[],ir:[],w:0,l:0,t:0,pf:0,pa:0,morale:50,streak:0,gmRep:50,chemistry:75,strat:pick(["balanced","pass-heavy","run-heavy","defensive"]),coach:{oc:genCoach("OC"),dc:genCoach("DC"),st:genCoach("ST")}}));}
...
if(injInsurance.includes(p.id)){t.deadCap=Math.max(0,(t.deadCap||0)-2);setInjInsurance(ins=>ins.filter(x=>x!==p.id));moraleEvLogs.push(`🛡️ INSURANCE: ${p.name} IR — $2M cap relief`);}else moraleEvLogs.push(`🚑 MAJOR INJURY: ${p.name} placed on IR (${p.injType})`);}}}));nt.forEach(t=>t.roster.forEach(p=>{p.av=calcAV(p);}));// PRACTICE SQUAD AUTO-RETURN
...
if(nw>=18){setSp("playoffs");const seedConf=(conf)=>{const ct=nt.filter(t=>TEAMS[t.id]?.c===conf);const divs={};ct.forEach(t=>{const dk=TEAMS[t.id]?.d||'X';(divs[dk]||(divs[dk]=[])).push(t);});Object.values(divs).forEach(d=>d.sort((a,b)=>b.w-a.w||(b.pf-b.pa)-(a.pf-a.pa)));const dw=Object.values(divs).map(d=>d[0]).sort((a,b)=>b.w-a.w||(b.pf-b.pa)-(a.pf-a.pa));const wc=ct.filter(t=>!dw.find(d=>d.id===t.id)).sort((a,b)=>b.w-a.w||(b.pf-b.pa)-(a.pf-a.pa)).slice(0,4-dw.length);return[...dw,...wc].slice(0,4).map(t=>t.id);};const af=seedConf('AFC'),nf=seedConf('NFC');setPb({rd:1,m:[[af[0],af[3]],[af[1],af[2]],[nf[0],nf[3]],[nf[1],nf[2]]],res:[],ch:null});setTab("playoffs");}};
  ...
  const goToCombine=()=>{const ndc={...dc};const cls=ndc[yr]||[];cls.forEach(p=>{p.combine=genCombine(p.pos,p.trueOvr);p.proDay=genProDay(p.pos,p.trueOvr);const ph=combToPhys(p.combine);Object.assign(p,ph);p.combGrade=p.trueOvr>=86?'A+':p.trueOvr>=80?'A':p.trueOvr>=74?'B':p.trueOvr>=68?'C':'D';});ndc[yr]=[...cls];setDc(ndc);setSp("combine");setTab("draft");sm("Combine complete!");};
