...
const PlayerTable=({players,setSel,sortCol,sortDir,onSort,posFilter,setPosFilter,showSign,onSign})=>{
  ...
  return(<div>
    ...
    <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}><thead><tr>
      ...
      <td style={{textAlign:"center",color:C.gd,fontSize:9}}>{typeof p.salary==="number"?`$${p.salary.toFixed(1)}`:"-"}{p.bonus&&<span style={{color:C.gd,fontSize:6}}>★</span>}</td>
...
nt.forEach((t,i)=>{if(capHit(t)>CAP_CEILING){t.deadCap=(t.deadCap||0)+5;if(i===ui){const pk3=draftPicks.find(pk=>pk.owner===ui&&pk.rd===3);if(pk3)setDraftPicks(pr=>pr.filter(p=>p.id!==pk3.id));setLog(l=>[`⚠️ CAP VIOLATION: ${t.city} over $${CAP_CEILING}M — $5M fine + ${pk3?'3rd-round pick forfeited':'no 3rd pick to forfeit'}`,...l.slice(0,149)]);}}});const ns=[...sched];let ur="";ns.filter(g=>g.wk===nw&&!g.played).forEach(g=>{const gi=ns.indexOf(g);let r=simGame(nt[g.h],nt[g.a],g.h===ui?gamePlan:{off:'balanced',def:'balanced'},g.a===ui?gamePlan:{off:'balanced',def:'balanced'});// FEATURE 2: Stadium effects
...
if(nw===18){const devLogs2=[];nt.forEach((t,ti)=>{t.roster.forEach(p=>{let bonus=0;const r=Math.random();if(p.age===21){if(r<0.10)bonus=2;else if(r<0.55)bonus=1;}else if(p.age===22){if(r<0.08)bonus=2;else if(r<0.48)bonus=1;}else if(p.age===23){if(r<0.05)bonus=2;else if(r<0.35)bonus=1;}else if(p.age===24){if(r<0.20)bonus=1;}else if(p.age<=26){if(r<0.10)bonus=1;}if(p.mentor){const mP=t.roster.find(r=>r.id===p.mentor);if(mP&&Math.random()<0.35)bonus=Math.max(bonus,1);}if(bonus>0){const no=Math.min(99,p.ovr+bonus);p.ovr=no;p.trueOvr=no;if(ti===ui)devLogs2.push(`📈 ${p.name} improved to OVR ${no} this offseason (${p.mentor?'mentored':'natural'})`);}});});if(devLogs2.length)setLog(l=>[...devLogs2,...l.slice(0,150-devLogs2.length)]);}
...
const aiSigned=new Set();nt.forEach((t,idx)=>{if(idx===ui)return;POS.forEach(pos=>{const have=t.roster.filter(p=>p.pos===pos).length;const need=(rm[pos]||2)-have;for(let s=0;s<Math.max(0,need);s++){const top=expFAs.find(f=>f.pos===pos&&!aiSigned.has(f.id));if(!top)break;aiSigned.add(top.id);t.roster.push({...top,salary:+(top.ovr/99*Rf(2,8)).toFixed(1),contract:R(1,3),ss:emptySS(top.pos),gl:[],av:0});}});});
...
nt[ui].chemistry=cl((nt[ui].chemistry||75)-trOff.g.length*2+trOff.r.length,0,100);setTeams(nt);setDraftPicks(np);setLog(p=>[...p,`Trade with ${TEAMS[trTm].ab}`]);sm("Trade completed!");setTrOff({g:[],r:[],gPk:[],rPk:[]});};
  const acceptAiOffer=()=>{if(!aiOffer)return;if(wk>=10&&sp==='regular'){sm("Trade deadline passed!");setAiOffer(null);return;}const nt=[...teams];const offered=aiOffer.offeredPids.map(pid=>nt[aiOffer.tid].roster.find(p=>p.id===pid)).filter(Boolean);const wanted=nt[ui].roster.find(p=>p.id===aiOffer.wantedPid);if(!wanted)return;offered.forEach(p=>{const i=nt[aiOffer.tid].roster.findIndex(r=>r.id===p.id);if(i>=0){const[rm]=nt[aiOffer.tid].roster.splice(i,1);nt[ui].roster.push(rm);}});const wi=nt[ui].roster.findIndex(r=>r.id===wanted.id);if(wi>=0){const[rm]=nt[ui].roster.splice(wi,1);nt[aiOffer.tid].roster.push(rm);}nt[ui].chemistry=cl((nt[ui].chemistry||75)-2+offered.length,0,100);setTeams(nt);setAiOffer(null);setLog(l=>[`🤝 ACCEPTED AI OFFER: Got ${offered.map(p=>p.name).join('+')} from ${TEAMS[aiOffer.tid].ab} for ${wanted.name}`,...l.slice(0,149)]);sm("Trade accepted!");};
  ...
  const fireCoach=(role)=>{const nt=[...teams];const fired=nt[ui].coach?.[role.toLowerCase()];if(!fired)return;nt[ui]={...nt[ui],coach:{...nt[ui].coach,[role.toLowerCase()]:null}};nt[ui].deadCap=+((nt[ui].deadCap||0)+(fired.salary*0.5)).toFixed(1);setFaCoaches(pr=>[...pr,fired]);setTeams(nt);setLog(l=>[`🔥 Fired ${fired.name} (${role.toUpperCase()}) — $${(fired.salary*0.5).toFixed(1)}M dead cap`,...l.slice(0,149)]);sm(`Fired ${fired.name}`);};
