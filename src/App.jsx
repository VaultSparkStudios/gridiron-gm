...
const[fourthChoice,setFourthChoice]=useState(null);const[showDepth,setShowDepth]=useState(false);const[showPS,setShowPS]=useState(false);const[showIR,setShowIR]=useState(false);const[preseasonGames,setPreseasonGames]=useState([]);
  ...
  const buyStadium=(key,cost)=>{if(scPts<cost){sm(`Need ${cost} SP!`);return;}if((stadium.upgrades||[]).includes(key)){sm("Already purchased!");return;}setStadium(s=>({...s,upgrades:[...(s.upgrades||[]),key]}));setScPts(p=>p-cost);sm(`Stadium upgrade: ${key} purchased!`);};
...
nt.forEach((t,i)=>{if(capHit(t)>CAP_CEILING){t.deadCap=(t.deadCap||0)+5;if(i===ui){const pk3=draftPicks.find(pk=>pk.owner===ui&&pk.rd===3);if(pk3)setDraftPicks(pr=>pr.filter(p=>p.id!==pk3.id));setLog(l=>[`⚠️ CAP VIOLATION: ${t.city} over $${CAP_CEILING}M — $5M fine + ${pk3?'3rd-round pick forfeited':'no 3rd pick to forfeit'}`,...l.slice(0,149)]);}}});const ns=[...sched];let ur="";ns.filter(g=>g.wk===nw&&!g.played).forEach(g=>{const gi=ns.indexOf(g);const r=simGame(nt[g.h],nt[g.a],g.h===ui?gamePlan:{off:'balanced',def:'balanced'},g.a===ui?gamePlan:{off:'balanced',def:'balanced'});ns[gi]={...g,played:true,hs:r.hsc,as:r.asc,boxH:r.boxH,boxA:r.boxA,weather:r.wx};nt[g.h].pf+=r.hsc;nt[g.h].pa+=r.asc;nt[g.a].pf+=r.asc;nt[g.a].pa+=r.hsc;const hWon=r.hsc>r.asc,aWon=r.asc>r.hsc;if(hWon){nt[g.h].w++;nt[g.a].l++;}else if(aWon){nt[g.a].w++;nt[g.h].l++;}else{nt[g.h].t++;nt[g.a].t++;}// Chemistry: win/loss for all teams
...
// FAN SAT UPDATES
let fsD=0;const uGame=ns.find(g=>g.wk===nw&&g.played&&(g.h===ui||g.a===ui));if(uGame){const isH=uGame.h===ui;const us=isH?uGame.hs:uGame.as;const them=isH?uGame.as:uGame.hs;if(us>them)fsD+=8;else fsD-=10;}
...
setNews([]);setTrainFocus(null);setNegPlayer(null);
...
if(d.passYds||d.pyds){p.ss.passYds=(p.ss.passYds||0)+(d.passYds||d.pyds||0);p.ss.passTD=(p.ss.passTD||0)+(d.passTD||d.ptds||0);p.ss.passInt=(p.ss.passInt||0)+(d.passInt||d.ints||0);p.ss.att=(p.ss.att||0)+(d.att||Math.ceil((d.passYds||d.pyds||0)/8));p.ss.comp=(p.ss.comp||0)+(d.comp||Math.ceil((d.passYds||d.pyds||0)/12));}if(d.rushYds||d.ryds){p.ss.rushYds=(p.ss.rushYds||0)+(d.rushYds||d.ryds||0);p.ss.rushAtt=(p.ss.rushAtt||0)+(d.rushAtt||Math.ceil((d.rushYds||d.ryds||0)/4));p.ss.rushTD=(p.ss.rushTD||0)+(d.rushTD||d.rtds||0);}if(d.recYds||d.reyds){p.ss.recYds=(p.ss.recYds||0)+(d.recYds||d.reyds||0);p.ss.rec=(p.ss.rec||0)+(d.rec||Math.ceil((d.recYds||d.reyds||0)/9));p.ss.recTD=(p.ss.recTD||0)+(d.recTD||d.retds||0);}if(d.tack){p.ss.tkl=(p.ss.tkl||0)+d.tack;}if(d.sacks){p.ss.sacks=(p.ss.sacks||0)+d.sacks;}p.ss.gp=(p.ss.gp||0)+1;liveStatsSynced=true;}});(res.injuries||[]).forEach(inj=>{const p=nt[ui].roster.find(r=>r.id===inj.id);if(p&&!p.injured){p.injured=true;p.injWk=inj.weeks;p.injType="Game injury";}});const ts=res.score?.team||0,os=res.score?.opp||0;if(ts>os)nt[ui].w++;else if(os>ts)nt[ui].l++;else nt[ui].t++;nt[ui].pf+=ts;nt[ui].pa+=os;setTeams(nt);// Mark the matching scheduled game as played
  ...
  const saveGame=()=>{const d={v:2,yr,wk,sp,ui,teams,sched,byeMap,fa,waivers,dc,draftPicks,draftIdx,draftLog,log,champs,pb,myScout,faScouts,scPts,faCoaches,rivalry,ftag,ownerGoals,news,fanSat,scoutBudget,coachMarket,stadium,ownerGoalHistory,ownerGoalStreak,awards,draftClassRating,aiOffer};const b=new Blob([JSON.stringify(d)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download=`gm-${yr}-wk${wk}.json`;a.click();sm("Saved!");};
