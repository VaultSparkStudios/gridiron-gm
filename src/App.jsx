...
function simGame(ht,at,hPlan,aPlan){const hs=teamStr(ht)+2.5,as=teamStr(at);
  ...
  const boxH={},boxA={};
  [ht,at].forEach((t,ti)=>{const box=ti===0?boxH:boxA;t.roster.forEach(p=>{if(p.injured)return;const gs=simPG(p);if(!Object.keys(gs).length)return;p.ss.gp=(p.ss.gp||0)+1;p.ss.gs=(p.ss.gs||0)+1;addS(p.ss,gs);if(p.pos==="QB"&&p.ss.att>0)p.ss.rate=qbRate(p.ss.comp,p.ss.att,p.ss.passYds,p.ss.passTD,p.ss.passInt);p.gl.push({...gs,gp:1});box[p.id]={...gs,name:p.name,pos:p.pos};if(Math.random()<.025){p.injured=true;p.injType=pick(["Hamstring","Ankle","Knee (MCL)","Shoulder","Concussion","Quad","Calf","Back"]);const sevRoll=Math.random();if(sevRoll<0.4){p.injSev="minor";p.injRecWks=R(1,2);}else if(sevRoll<0.8){p.injSev="moderate";p.injRecWks=R(3,5);}else{p.injSev="major";p.injRecWks=R(6,8);}p.injWk=p.injRecWks;}});});
...
const holdout=t.roster.find(p=>p.contract===1&&p.ovr>=75);if(holdout&&Math.random()<0.04){holdout.conf=Math.max(0,(holdout.conf||50)-12);t.morale=cl((t.morale||50)-4,0,100);moraleEvLogs.push(`✋ HOLDOUT: ${holdout.name} (${holdout.pos}) in contract year — skipping practice`);}// Leadership Boost
